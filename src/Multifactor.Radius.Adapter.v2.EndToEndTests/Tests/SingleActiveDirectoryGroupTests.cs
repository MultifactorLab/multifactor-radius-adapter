...
public class SingleActiveDirectoryGroupTests(RadiusFixtures radiusFixtures) : E2ETestBase(radiusFixtures)
{
    ...
    [Theory]
    [InlineData("ad-root-conf.env")]
    public async Task BST007_NestedGroup_ShouldAccept(string configName)
    {
        var sensitiveData =
            E2ETestsUtils.GetConfigSensitiveData(configName);
        ...
        var rootConfig = CreateRadiusConfiguration(sensitiveData, sensitiveData.GetConfigValue("root", "NestedAccessGroups")!);
        ...
        var rootConfig = new RadiusAdapterConfiguration()
        {
            ...
            LdapServers = new LdapServersSection()
            {
                LdapServer = new LdapServerConfiguration()
                {
                    ...
                    AccessGroups = activeDirectoryGroup,
