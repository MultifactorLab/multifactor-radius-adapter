...
public class MultipleActiveDirectory2FaGroupsTests(RadiusFixtures radiusFixtures) : E2ETestBase(radiusFixtures)
{
    ...
    public async Task BST012_ShouldAccept(string configName)
    {
        ...
        var rootConfig = CreateRadiusConfiguration(sensitiveData, $"cn=Not,dc=Existed,dc=Group1;cn=Not,dc=Existed,dc=Group2;{sensitiveData.GetConfigValue("root", "AccessGroups")!}");
    ...
    [Theory]
    [InlineData("ad-root-conf.env")]
    public async Task BST012_NestedGroups_ShouldAccept(string configName)
    {
        var sensitiveData =
            E2ETestsUtils.GetConfigSensitiveData(configName);
        ...
        var rootConfig = CreateRadiusConfiguration(sensitiveData, $"cn=Not,dc=Existed,dc=Group1;cn=Not,dc=Existed,dc=Group2;{sensitiveData.GetConfigValue("root", "NestedAccessGroups")!}");
    ...
    private RadiusAdapterConfiguration CreateRadiusConfiguration(
        ...
        string activeDirectory2FaGroups)
    {
        var configName = "root";
        var rootConfig = new RadiusAdapterConfiguration()
        {
            ...
            LdapServers = new LdapServersSection()
            {
                LdapServer = new LdapServerConfiguration()
                {
                    ...
                    SecondFaGroups = activeDirectory2FaGroups,
