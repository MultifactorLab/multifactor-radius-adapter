...
{
    private readonly IChallengeContextCache _cache; //todo new srv
    ...
    public SecondFactorChallengeProcessor(
        ...
        IChallengeContextCache memoryCache)
    {
    ...
    public ChallengeIdentifier AddChallengeContext(RadiusPipelineContext context)
    {
        ...
        var identifier = new ChallengeIdentifier(context.ClientConfiguration.Name, context.ResponseInformation.State);
        var key = identifier.ToString();        
        try
        {
            _cache.Set(key, context);
            return identifier;
        }
        catch (Exception ex)
        {
    ...
    private RadiusPipelineContext? GetChallengeContext(ChallengeIdentifier identifier)
    {
        ...
        var key = identifier.ToString();        
        if (_cache.TryGetValue(key, out RadiusPipelineContext? context))
        {
    ...
    private bool ShouldCacheResponse(RadiusPipelineContext context)
    {
        ...
        if (context.LdapProfile != null && context.LdapProfile.MemberOf.Intersect(cacheGroups).Any()) return true;
        var isMember = _checkMembership.Execute(MembershipDto.FromContext(context, cacheGroups));
