...
internal sealed class UserNameValidationStep : IRadiusPipelineStep
{
    ...
    private const string StepName = nameof(UserNameValidationStep);
    ...
    public Task ExecuteAsync(RadiusPipelineContext context)
    {
        ...
        if (identity.Format != UserIdentityFormat.UserPrincipalName 
            || IsPermittedSuffix(identity.GetUpnSuffix(), 
                                serverSettings.IncludedSuffixes, 
                                serverSettings.ExcludedSuffixes,
                                context.ForestMetadata))
            return Task.CompletedTask;
