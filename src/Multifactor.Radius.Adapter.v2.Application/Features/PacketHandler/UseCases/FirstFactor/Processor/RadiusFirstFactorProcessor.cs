...
internal sealed class RadiusFirstFactorProcessor : IFirstFactorProcessor
{
    ...
    public RadiusFirstFactorProcessor(IRadiusClientFactory radiusClientFactory, 
        ILogger<RadiusFirstFactorProcessor> logger, IPacketSerializer packetSerializer, IPacketParser packetParser)
    {
    ...
    public async Task Execute(RadiusPipelineContext context)
    {
        ...
        if (string.IsNullOrWhiteSpace(requestPacket.UserName))
        {
            _logger.LogWarning("Can't find User-Name in message id={id} from {host:l}:{port}", 
                context.RequestPacket.Identifier, context.RequestPacket.RemoteEndpoint?.Address, 
        ...
        try
        {
            ...
            var authBytes = _packetSerializer.Execute(authPacket, new SharedSecret(context.ClientConfiguration.RadiusSharedSecret));
            ...
            {
                response = await SendRequestToNpsServer(client, npsEndPoint, authPacket.Identifier, 
                    requestPacket.Identifier, authBytes, context.ClientConfiguration.NpsServerTimeout);
            ...
            var responsePacket = _packetParser.Execute(response, 
                new SharedSecret(context.ClientConfiguration.RadiusSharedSecret), authPacket.Authenticator);
            _logger.LogDebug("Received {code:l} message with id={id} from Remote Radius Server", 
                authPacket.Code.ToString(), authPacket.Identifier);

            if (responsePacket.Code == PacketCode.AccessAccept)
            {
                var userName = requestPacket.UserName;
                _logger.LogInformation("User '{user:l}' credential and status verified successfully at {endpoint:l}", 
                    userName, endPoint);
            }
    ...
    private async Task<byte[]?> SendRequestToNpsServer(IRadiusClient client, IPEndPoint npsServerEndpoint, 
        byte authIdentifier, byte requestIdentifier, byte[] payload, TimeSpan timeout)
    {
    ...
    {
        var authPacket = new RadiusPacket(new RadiusPacketHeader(radiusPacket.Code, radiusPacket.Identifier, 
            radiusPacket.Authenticator));
