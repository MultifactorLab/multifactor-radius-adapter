...
using static Multifactor.Radius.Adapter.v2.Core.Interop.NativeMethods;
...
namespace Multifactor.Radius.Adapter.v2.Core.Interop
{
    public class NameTranslator : IDisposable
    {
        ...
        public NameTranslator(string domain, ILogger logger)
        {
            ...
            {
                _logger.LogWarning($"Failed to bind to: {domain}");
        ...
        public string Translate(string netbiosName)
        {
            ...
            {
                _logger.LogWarning($"Failed to translate {netbiosName} in {_domain}");
            ...
            try
            {
                ...
                {
                    _logger.LogWarning($"Unexpected result of translation {netbiosName} in {_domain}");
