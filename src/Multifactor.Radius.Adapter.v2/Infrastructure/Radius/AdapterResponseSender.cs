...
public class AdapterResponseSender : IResponseSender
{
    ...
    private static void AddResponsePacketAttributes(IRadiusPacket? source, RadiusPacket target)
    {
        if (source is null)
            return;
    ...
    private static void AddProxyAttribute(IRadiusPacket source, RadiusPacket target)
    {
    ...
    private static PacketCode ToPacketCode(AuthenticationState authenticationState)
    {
    ...
    private async Task SendResponse(IRadiusPacket responsePacket, SendAdapterResponseRequest request)
    {
        var bytes = _radiusPacketService.GetBytes(responsePacket, request.RadiusSharedSecret);
        var endpoint = request.ProxyEndpoint ?? request.RemoteEndpoint;
        if (responsePacket.Code == PacketCode.AccessReject)
            await RandomWaiter.WaitSomeTimeAsync(request.InvalidCredentialDelay.ZeroDelay,
                request.InvalidCredentialDelay.Min, request.InvalidCredentialDelay.Max);
