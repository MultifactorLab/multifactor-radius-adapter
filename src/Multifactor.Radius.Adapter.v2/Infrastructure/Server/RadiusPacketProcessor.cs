...
public class RadiusPacketProcessor : IRadiusPacketProcessor
{
    ...
    public async Task ProcessAsync(IRadiusPacket requestPacket, IClientConfiguration clientConfiguration)
    {
        ...
        {
            var isSuccessful = await ExecutePipeline(clientConfiguration, requestPacket, serverConfig);
            if (isSuccessful)
                return;
        }
    }
    ...
    private SendAdapterResponseRequest GetResponseRequest(RadiusPipelineExecutionContext context) => new(context);
}