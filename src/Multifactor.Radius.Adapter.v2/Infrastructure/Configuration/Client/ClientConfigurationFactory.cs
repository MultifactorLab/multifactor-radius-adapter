...
using NetTools;
using AppSettingsSection = Multifactor.Radius.Adapter.v2.Domain.RadiusAdapter.Sections.AppSettingsSection;
using LdapServerConfiguration = Multifactor.Radius.Adapter.v2.Infrastructure.Configuration.Client.LdapServerConfiguration;
        ...
        if (radiusReplyAttributesSection != null)
        {
            foreach (var attribute in radiusReplyAttributesSection.Attributes.Elements)
            {
                ...
                if (!replyAttributes.ContainsKey(attribute.Name))
                    replyAttributes.Add(attribute.Name, []);
