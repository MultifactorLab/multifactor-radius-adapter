...
После этого адаптер получает переменные из окружения и "накладывает" их поверх настроек, считанных из файлов.  
Таким образом выходит, что значения из переменных окружения перегружают значения из файлов настроек.  
